...
class TrackingWindow(QtWidgets.QWidget):
    ...
    def __init__(self):
        ...
        # Temp button to take screenshot
        screenshot_widget = QtWidgets.QPushButton("take a screenshot")
        screenshot_widget.clicked.connect(self.take_screen_shot)
        print("screenshot")
        screenshot_widget.setFixedWidth(200)
        ...
        self.setWindowFlags(QtCore.Qt.FramelessWindowHint | QtCore.Qt.WindowSystemMenuHint | QtCore.Qt.WindowStaysOnTopHint)
    ...
    def _handle_gaze_in_screen_stream(self, _timestamp, xpos, ypos):
        ...
        if math.isnan(xpos) or math.isnan(ypos):
            return
        time = self.slowDownFactor % 5
        if(time != 0):
            self.slowDownFactor+=1
            return
        else:
            self.slowDownFactor+=1
    ...
    def take_screen_shot(self):
        #print("taking a screenshot around "+str(self._xcoord)+", "+ str(self._ycoord))
        img = pyautogui.screenshot()
        pyautogui.screenshot("my_image.jpg")
        return img
        #img.save("my_image.png", 'PNG')
        # TO-DO: 
        # - Pass iamge to image-to-text API
        # - Process array of word and its area into kd-tree/array
